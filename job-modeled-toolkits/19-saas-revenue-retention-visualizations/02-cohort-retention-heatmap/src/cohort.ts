...
interface CohortCell {
  offset: number; // months since the cohort's signup month
  hasData: boolean; // false when the calendar month is past the end of the ledger
  retainedCents: number; // cohort revenue still active that month
  activeLogos: number; // cohort customers still active that month
  revenuePct: number; // retainedCents over the cohort's starting revenue, as a percent
  logoPct: number; // activeLogos over the cohort's starting count, as a percent
}

interface CohortSeries {
  cohort: string; // "YYYY-MM"
  startCents: number; // cohort revenue at offset 0
  cohortSize: number; // number of customers in the cohort
  cells: CohortCell[];
}
...
function lastMonthIn(rows: LedgerRow[]): string {
...
function cohortsIn(rows: LedgerRow[]): string[] {
 ...
 */
function computeCohorts(rows: LedgerRow[]): CohortSeries[] {
...
function toCohortCsv(series: CohortSeries[]): string {

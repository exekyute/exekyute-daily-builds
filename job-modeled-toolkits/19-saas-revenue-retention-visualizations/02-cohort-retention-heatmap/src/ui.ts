...
let currentCohorts: CohortSeries[] = [];
let mode: "revenue" | "logo" = "revenue";
...
function drawHeatmap(): void {
  const grid = el<HTMLDivElement>("heatmap");
  grid.innerHTML = "";
...
function handleFile(file: File): void {
  ...
  reader.onload = () => {
    try {
      ...
      el<HTMLDivElement>("heatmap").innerHTML = "";
...
function init(): void {
  ...
  el<HTMLSelectElement>("modePick").addEventListener("change", (event) => {
    mode = (event.target as HTMLSelectElement).value === "logo" ? "logo" : "revenue";
    if (currentCohorts.length > 0) {
      redraw();
    }
  });
  el<HTMLButtonElement>("exportBtn").addEventListener("click", exportCohorts);
}

...
function drawRetentionChart() {
    ...
    const yOf = (v) => plotH - (v / maxPct) * plotH;
...
function drawRetentionTable() {
    const tbody = el("retentionBody");
    tbody.innerHTML = "";
...
function drawRenewalsChart(buckets) {
    const svg = el("renewalChart");
    while (svg.firstChild) {
        svg.removeChild(svg.firstChild);
    }
    if (buckets.length === 0) {
    ...
    const yOf = (v) => plotH - (v / maxVal) * plotH;
    const group = svgEl("g", { transform: `translate(${margin.left},${margin.top})` });
    svg.appendChild(group);
    const ticks = 4;
    for (let t = 0; t <= ticks; t++) {
...
function drawRenewalsTable(buckets) {
    const tbody = el("renewalBody");
    tbody.innerHTML = "";
...
function drawSummary(buckets) {
    ...
    el("summary").innerHTML = parts.length > 0 ? parts.join("") : `<div class="stat"><span class="stat-value">--</span><span class="stat-label">load the movement file to begin</span></div>`;
}
function fillAsOf() {
    const select = el("asOf");
    select.innerHTML = "";
...
function renderRenewals() {
    ...
    const asOf = el("asOf").value || (movement.length > 0 ? movement[movement.length - 1].month : "2025-05");
    const horizon = Math.max(1, Number(el("horizon").value) || 3);
...
function handleMovementFile(file) {
...
function handleRenewalsFile(file) {
    const reader = new FileReader();
    reader.onload = () => {
        try {
...
function init() {
    el("movementInput").addEventListener("change", (event) => {
        const input = event.target;
        if (input.files && input.files.length > 0) {
            handleMovementFile(input.files[0]);
        }
    });
    el("renewalInput").addEventListener("change", (event) => {
        const input = event.target;
        if (input.files && input.files.length > 0) {
            handleRenewalsFile(input.files[0]);
        }
    });
    el("asOf").addEventListener("change", renderRenewals);
    el("horizon").addEventListener("change", renderRenewals);
}

...
test("per-line total for Auto is 47,500.00 over 73,000.00 premium", () => {
    const t = sampleGrid().lineTotals.get("Auto");
...
test("per-year total for 2023 is 60.0%", () => {
    const t = sampleGrid().periodTotals.get("2023");
...
test("parser rejects inconsistent premium within a line and year", () => {
    let message = "";
    const bad = SAMPLE_CLEAN.replace("A-2402,Auto,2024,2024-12-01,2024-12-31,12,open,,500.00,2000.00,2500.00,18000.00,Y,30,0-30,", "A-2402,Auto,2024,2024-12-01,2024-12-31,12,open,,500.00,2000.00,2500.00,19000.00,Y,30,0-30,");
    try {
        parseCleanCsv(bad);
    }
    catch (err) {

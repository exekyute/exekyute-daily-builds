...
const config: DashboardConfig = { intervalMinutes: 30, targetSlPct: 80 };

function headlineRow(): ActualRow {
  return {
    interval: "08:00",
    callsOffered: 98,
    callsAnswered: 96,
...
test("coverage joins the plan by interval", () => {
  const plan = new Map<string, number>([["08:00", 14]]);
...
test("understaffing shows as negative coverage", () => {
  const plan = new Map<string, number>([["10:00", 40]]);
  const row: ActualRow = {
    interval: "10:00",
    callsOffered: 305,
    callsAnswered: 270,

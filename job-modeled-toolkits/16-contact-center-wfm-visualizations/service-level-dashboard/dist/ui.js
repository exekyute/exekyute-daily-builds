 ...
 * breach colour.
 */
function drawServiceLevel(metrics, targetSlPct) {
    const svg = el("slChart");
    clear(svg);
    const width = 880;
    const height = 320;
    ...
    const yScale = (pct) => plotH - (pct / 100) * plotH;
 ...
 */
function drawCoverage(metrics) {
    const section = el("coverageSection");
    ...
    const plotH = height - margin.top - margin.bottom;
    svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
    const maxAgents = Math.max(1, ...metrics.map((m) => { var _a; return Math.max(m.agentsScheduled, (_a = m.requiredAgents) !== null && _a !== void 0 ? _a : 0); }));
    const yScale = (v) => plotH - (v / maxAgents) * plotH;
    const group = svgEl("g", { transform: `translate(${margin.left},${margin.top})` });
    svg.appendChild(group);
    for (let t = 0; t <= 4; t++) {
    ...
    metrics.forEach((m, i) => {
        var _a;
        const cx = i * slotW + slotW / 2;
        const required = (_a = m.requiredAgents) !== null && _a !== void 0 ? _a : 0;
        const reqY = yScale(required);
        group.appendChild(svgEl("rect", { x: cx - barW, y: reqY, width: barW, height: plotH - reqY, class: "bar-required" }));
...
function drawSummary(metrics, rows) {
...
function drawTable(metrics) {
    ...
    for (const m of metrics) {
        ...
        const coverageText = m.coverage === null ? "--" : m.coverage > 0 ? `+${m.coverage}` : String(m.coverage);
        const cells = [
            m.interval,
            String(m.offered),
            `${m.slPct.toFixed(2)}%`,
...
function init() {
    el("actualsInput").addEventListener("change", (event) => {
        const input = event.target;
        if (input.files && input.files.length > 0) {
            handleActuals(input.files[0]);
        }
    });
    el("planInput").addEventListener("change", (event) => {
        const input = event.target;
        if (input.files && input.files.length > 0) {

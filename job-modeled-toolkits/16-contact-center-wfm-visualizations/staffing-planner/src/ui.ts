...
let currentResults: StaffingResult[] = [];
...
function readConfig(): PlannerConfig {
  return {
    intervalMinutes: Number(el<HTMLInputElement>("intervalMinutes").value),
    targetSlPct: Number(el<HTMLInputElement>("targetSlPct").value),
    targetAnswerSeconds: Number(el<HTMLInputElement>("targetAnswerSeconds").value),
    shrinkagePct: Number(el<HTMLInputElement>("shrinkagePct").value),
  };
}
 ...
 */
function drawChart(results: StaffingResult[]): void {
  ...
  const agentScale = (v: number): number => plotH - (v / maxAgents) * plotH;
  const slScale = (pct: number): number => plotH - (pct / 100) * plotH;
  ...
  results.forEach((r, i) => {
    group.appendChild(
      svgEl("circle", { cx: i * slotW + slotW / 2, cy: slScale(r.projectedSlPct), r: 3, class: "sl-dot" }),
    );
  });
}

function drawTable(results: StaffingResult[]): void {
  const tbody = el<HTMLTableSectionElement>("planBody");
  tbody.innerHTML = "";
...
function drawSummary(results: StaffingResult[], config: PlannerConfig): void {
...
function render(forecasts: IntervalForecast[]): void {
...
let loadedForecasts: IntervalForecast[] = [];
...
function init(): void {
  ...
  el<HTMLButtonElement>("exportBtn").addEventListener("click", exportPlan);
}

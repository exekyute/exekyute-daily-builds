 ...
 */
function classify(asOf, dueDate) {
 ...
 */
function parseObligationCsv(text) {
    const lines = text
        .split(/\r?\n/)
        .map((line) => line.trim())
...
function isOverdue(asOf, obligation) {
 ...
 */
function buildLadder(obligations, asOf, thresholdCents) {
    const buckets = [];
    ...
    const indexOfWeek = (w) => w; // overdue at 0, week w at index w
 ...
 */
function toMaturitiesByWeekCsv(obligations, asOf) {
    const weekTotals = new Array(WEEKS + 1).fill(0); // 1..13 used
    for (const ob of obligations) {
        const place = classify(asOf, ob.dueDate);
        if (place.kind === "overdue") {

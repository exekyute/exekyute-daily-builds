...
function readAsOf() {
    return el("asOf").value;
}
function readThresholdCents() {
    const raw = el("threshold").value.trim();
    if (!/^\d+(\.\d{1,2})?$/.test(raw)) {
...
function drawTable(buckets) {
    const tbody = el("ladderBody");
    tbody.innerHTML = "";
...
function drawSummary(summary, asOf) {
...
function init() {
    ...
    el("exportBtn").addEventListener("click", exportMaturities);
}

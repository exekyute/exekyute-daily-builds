...
interface WeekFlow {
  week: number; // 1..13
  label: string; // a short note for the week, e.g. "Wk of Jun 15"
  operatingInflowCents: number;
  operatingOutflowCents: number;
}

interface ForecastConfig {
  openingCashCents: number; // may be negative
  minimumBufferCents: number; // the floor cash must stay above
}
...
interface WeekResult {
  ...
  netCents: number;
  closingCents: number;
  headroomCents: number; // closing minus buffer; negative means a breach
  breach: boolean;
}
...
interface ForecastSummary {
  ...
  lowestWeek: number;
  breachCount: number;
  firstBreachWeek: number; // 0 when there is no breach
}
 ...
 */
function sumClosingBalancesCsv(text: string): number {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
 ...
 */
function parseOperatingCsv(text: string): WeekFlow[] {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.length > 0);
  ...
  const byWeek = new Map<number, WeekFlow>();
  for (let i = 1; i < lines.length; i++) {
    const rowNumber = i + 1;
    const cells = lines[i].split(",").map((c) => c.trim());
    if (cells.length !== 4) {
 ...
 */
function parseMaturitiesCsv(text: string): Map<number, number> {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.length > 0);
 ...
 */
function runForecast(flows: WeekFlow[], debtByWeek: Map<number, number>, config: ForecastConfig): WeekResult[] {
  const results: WeekResult[] = [];
  let opening = config.openingCashCents;
  for (const flow of flows) {
    const debtDue = debtByWeek.get(flow.week) ?? 0;
...
function summarize(results: WeekResult[]): ForecastSummary {
...
function toForecastCsv(results: WeekResult[]): string {

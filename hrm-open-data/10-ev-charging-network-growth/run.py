"""Thin driver for the EV charging network growth pipeline.

All analytical logic lives in sql/. This file only executes the SQL files in
order (00 to 99), copies the frozen mart into bi/exports/, and diffs the
generated output against the golden copies in expected/. The SQL writes four
files: the per-charger mart and three aggregate tables.

  python run.py            run the pipeline, then verify against expected/
  python run.py verify     golden diff only
  python run.py show        print the growth curve and the two mix breakdowns
"""
...
MART_CSV = os.path.join(OUT_DIR, "mart_ev.csv")
YEAR_CSV = os.path.join(OUT_DIR, "chargers_by_year.csv")
CHARTYPE_CSV = os.path.join(OUT_DIR, "counts_by_chartype.csv")
CONNECTYPE_CSV = os.path.join(OUT_DIR, "counts_by_connectype.csv")
BI_MART_CSV = os.path.join(HERE, "bi", "exports", "mart_ev.csv")
...
def run_pipeline():
    ...
    print("copied mart to bi/exports/mart_ev.csv")
...
def show():
    year_src = YEAR_CSV if os.path.exists(YEAR_CSV) \
        else os.path.join(EXPECTED_DIR, "chargers_by_year.csv")
    chartype_src = CHARTYPE_CSV if os.path.exists(CHARTYPE_CSV) \
        else os.path.join(EXPECTED_DIR, "counts_by_chartype.csv")
    connectype_src = CONNECTYPE_CSV if os.path.exists(CONNECTYPE_CSV) \
        else os.path.join(EXPECTED_DIR, "counts_by_connectype.csv")
    if not os.path.exists(year_src):
        print("Nothing to show yet. Run: python run.py")
        return 1

    print("Chargers installed by year, with the running cumulative network size")
    print("")
    header, rows = read_csv_rows(year_src)
    print_table(header, rows)

    print("")
    print("Chargers by charging level")
    print("")
    header, rows = read_csv_rows(chartype_src)
    print_table(header, rows)

    print("")
    print("Chargers by connector type")
    print("")
    header, rows = read_csv_rows(connectype_src)
    print_table(header, rows)
    print("")
    return 0

"""Thin driver for the arena booking utilization pipeline.

This file holds no analytical logic. It executes the SQL files in order (00 to 99),
which do all of the loading, cleaning, aggregating, and exporting, then it diffs the
generated output against the golden copy in expected/. The SQL writes two files: the
golden facility-month table in out/ and the frozen use-type mart in bi/exports/.
...
Usage:
    ...
    python run.py show       print the facility-month result as an aligned table
"""
...
DB_PATH = os.path.join(HERE, "arena.duckdb")
OUT_PATH = os.path.join(HERE, "out", "arena_utilization.csv")
EXPECTED_PATH = os.path.join(HERE, "expected", "arena_utilization.csv")
BI_MART_PATH = os.path.join(HERE, "bi", "exports", "mart_arena.csv")
...
def verify():
    ...
    if not os.path.exists(OUT_PATH):
        print("FAIL: out/arena_utilization.csv does not exist. Run: python run.py")
        return False
    if not os.path.exists(EXPECTED_PATH):
        print("FAIL: expected/arena_utilization.csv does not exist.")
        return False
    ...
    if actual == expected:
        print("PASS: out/arena_utilization.csv matches expected/ ({} rows).".format(
            max(len(actual) - 1, 0)))
        return True
...
def build():
    ...
    try:
        ...
        for (line,) in con.execute("SELECT line FROM headline ORDER BY ord").fetchall():
            print(line)
        print("")
        print("wrote bi/exports/mart_arena.csv")
        print("")
    finally:
        con.close()
...
def show():
    ...
    try:
        ...
        print("\nArena utilization by facility and month (busiest facility-months "
              "first):\n")
        summary = con.sql(
            "SELECT facility, month_start, bookings, booked_hours, ice_hours, "
            "dry_floor_hours, other_hours, ice_share, utilization "
            "FROM result ORDER BY booked_hours DESC, facility, month_start LIMIT 25")
        print_table(summary.columns, summary.fetchall())
        print("\n(showing the 25 busiest facility-months; the full golden has one row "
              "per facility and month)\n")
    finally:
        con.close()

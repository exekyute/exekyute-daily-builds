"""Thin driver for the land use composition pipeline.

All analytical logic lives in sql/. This file only executes the SQL files in
order, copies the frozen mart into bi/exports/, and diffs the generated output
against the golden copies in expected/.

  python run.py            run the pipeline, then verify against expected/
  python run.py verify     golden diff only
  python run.py show       print the municipal composition and most mixed communities
"""
...
MART_CSV = os.path.join(OUT_DIR, "mart_landuse.csv")
OVERALL_CSV = os.path.join(OUT_DIR, "class_area_overall.csv")
RANKING_CSV = os.path.join(OUT_DIR, "community_mix_ranking.csv")
BI_MART_CSV = os.path.join(HERE, "bi", "exports", "mart_landuse.csv")
...
def run_pipeline():
    ...
    print("copied mart to bi/exports/mart_landuse.csv")
    print("wrote bi/exports/zoning_tagged.geojson")
...
def show():
    overall_src = OVERALL_CSV if os.path.exists(OVERALL_CSV) \
        else os.path.join(EXPECTED_DIR, "class_area_overall.csv")
    rank_src = RANKING_CSV if os.path.exists(RANKING_CSV) \
        else os.path.join(EXPECTED_DIR, "community_mix_ranking.csv")
    if not os.path.exists(overall_src):
        print("Nothing to show yet. Run: python run.py")
        return 1

    print("Land use share across HRM (largest first)")
    print("")
    header, rows = read_csv_rows(overall_src)
    print_table(header, rows)

    print("")
    print("Most mixed communities (top 15 by mix index)")
    print("")

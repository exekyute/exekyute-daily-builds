...
function parseActualsCsv(text) {
    ...
    const expectedHeader = "interval,calls_offered,calls_answered,answered_within_threshold,total_wait_seconds,total_handle_seconds,agents_scheduled";
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (answeredWithinThreshold > callsAnswered) {
            throw new Error(`Row ${rowNumber}: answered_within_threshold (${answeredWithinThreshold}) cannot exceed calls_answered (${callsAnswered}).`);
        }
        rows.push({
            interval,
            callsOffered,
...
function wholeNonNegative(raw, rowNumber, field) {
...
function numberNonNegative(raw, rowNumber, field) {
    const value = Number(raw);
...
function computeInterval(row, plan, config) {
    const intervalSeconds = config.intervalMinutes * 60;
    const requiredAgents = plan && plan.has(row.interval) ? plan.get(row.interval) : null;
    ...
    const occupancyPct = row.agentsScheduled === 0 ? 0 : roundTo((row.totalHandleSeconds / (row.agentsScheduled * intervalSeconds)) * 100, 2);
    return {
        interval: row.interval,
...
function computeMetrics(rows, plan, config) {
...
function summarize(metrics, rows) {

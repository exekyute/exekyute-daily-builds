...
function drawParetoChart(result) {
    ...
    rows.forEach((r, i) => {
        ...
        group.appendChild(svgEl("rect", { x: cx - barW / 2, y: plotH - barH, width: barW, height: Math.max(1, barH), rx: 3, class: r.vitalFew ? "bar vital" : "bar tail" }));
...
function drawParetoTable(result) {
    const tbody = el("paretoBody");
    tbody.innerHTML = "";
...
function drawParetoSummary(result) {
    el("paretoSummary").innerHTML = `
...
function drawSavingsChart(result) {
    const svg = el("savingsChart");
    clearSvg(svg);
    const rows = result.rows;
    const width = 880;
    const height = 360;
...
function drawSavingsTable(result) {
    const tbody = el("savingsBody");
    tbody.innerHTML = "";
    for (const r of result.rows) {
        const tr = document.createElement("tr");
        if (!r.met) {
...
function drawSavingsSummary(result) {
...
function drawSavingsWarnings(warnings) {
    const panel = el("savingsWarnings");
...
function wireInput(inputId, messageId, render) {
    el(inputId).addEventListener("change", (event) => {

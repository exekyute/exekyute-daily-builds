...
interface ComplianceLine {
  ...
  matched: boolean; // passes the three-way match
  offContract: boolean;
  reasons: string[]; // why the line is flagged, empty when fully compliant
}
...
function toleranceFor(poCents: number): number {
...
function judgeLine(input: {
  ...
  const maxDiff = Math.max(
    Math.abs(input.poCents - input.invoiceCents),
    Math.abs(input.invoiceCents - input.receivedCents),
    Math.abs(input.poCents - input.receivedCents),
  );
 ...
 */
function parseAndJudge(text: string): ComplianceLine[] {
  const rawLines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function summarize(lines: ComplianceLine[]): ComplianceSummary {
...
function flaggedOnly(lines: ComplianceLine[]): ComplianceLine[] {

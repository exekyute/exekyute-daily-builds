...
function drawBars(svgId, rows) {
    ...
    rows.forEach((r, i) => {
        ...
        group.appendChild(svgEl("rect", { x: 0, y, width: barW, height: barH, rx: 4, class: r.flagged ? "bar flagged" : "bar" }));
        const value = svgEl("text", { x: barW + 10, y: y + barH / 2 + 4, class: "bar-value" });
        value.textContent = `${money(r.cents)}  (${r.pct.toFixed(2)}%)`;
        group.appendChild(value);
    });
}
function drawTable(bodyId, rows, firstHeader) {
...
function render(text) {
    ...
    drawSummary(summary);
    const categoryRows = totalsByCategory(lines).map((c) => ({ label: c.category, cents: c.cents, pct: c.pct, flagged: !c.known }));
    const supplierRows = totalsBySupplier(lines).map((s) => ({ label: s.supplier, cents: s.cents, pct: s.pct, flagged: false }));
...
function init() {
    ...
    el("exportBtn").addEventListener("click", exportNormalized);
}

...
function drawGrid(grid: LossRatioGrid): void {
  const table = el<HTMLTableElement>("grid");
  table.innerHTML = "";
  ...
  for (const line of grid.lines) {
    ...
    const lineTotal = grid.lineTotals.get(line) as Cell;
  ...
  for (const period of grid.periods) {
    footRow.appendChild(ratioCell(grid.periodTotals.get(period) as Cell, true));
  }
...
function ratioCell(cell: Cell, isTotal = false): HTMLTableCellElement {
...
function drawSummary(grid: LossRatioGrid): void {
...
function bestOrWorst(grid: LossRatioGrid, wantHighest: boolean): { label: string; ratio: number } {
  let pick: { label: string; ratio: number } | null = null;
  for (const line of grid.lines) {
    for (const period of grid.periods) {
      const cell = grid.cells.get(`${line}|${period}`);
      if (!cell) {

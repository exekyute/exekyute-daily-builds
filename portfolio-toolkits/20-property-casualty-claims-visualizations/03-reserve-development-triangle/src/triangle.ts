...
interface ClaimPaidRow {
  line: string;
  accidentPeriod: string;
  developmentMonth: number;
  paidCents: number; // cumulative paid to date at this development month
}

interface AgeToAge {
  from: number; // development month
  to: number; // the next development month
  factor: number; // sum of cumulative paid at "to" / sum at "from", over shared years
}

interface Projection {
  period: string;
  latestDev: number; // the most mature development month for this year
  latestPaidCents: number; // cumulative paid at that development month
  cdf: number; // cumulative development factor from latestDev to ultimate
  ultimateCents: number; // latest paid times the factor, rounded to the cent
  reserveCents: number; // ultimate minus latest paid
}

interface Triangle {
  lineFilter: string; // "All" or a specific line
  periods: string[]; // accident years, ascending
  devMonths: number[]; // development months, ascending
  cells: Map<string, number>; // key "period|dev" -> cumulative paid cents
  ageToAge: AgeToAge[]; // one per adjacent development-month pair
  cdf: Map<number, number>; // development month -> factor to ultimate
 ...
 */
function parseCleanCsv(text: string): ClaimPaidRow[] {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function linesIn(rows: ClaimPaidRow[]): string[] {
 ...
 */
function buildTriangle(rows: ClaimPaidRow[], lineFilter: string): Triangle {
  ...
  const projections: Projection[] = periods.map((period) => {
    let latestDev = -1;
    ...
    const factor = cdf.get(latestDev) as number;

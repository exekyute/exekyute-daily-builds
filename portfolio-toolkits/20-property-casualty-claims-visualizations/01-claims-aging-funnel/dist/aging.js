...
const HEADER = "claim_id,line_of_business,accident_period,report_date,valuation_date,development_month,status,close_date,paid_to_date,case_reserve,earned_premium";
...
function parseMoneyToCents(raw, rowNumber, field, minPositive) {
    if (!/^\d+(\.\d{1,2})?$/.test(raw)) {
...
function parseRegisterCsv(text) {
    ...
    const seenValuation = new Set(); // claim_id + valuation_date, to catch duplicates
    const claimMeta = new Map(); // claim_id -> "line|accidentPeriod|reportDate"
    const premiumByGroup = new Map(); // line|accidentPeriod -> premium cents
    const rows = [];
...
function summarize(rows) {
    ...
    const bucketCounts = new Map(BUCKET_LABELS.map((label) => [label, 0]));
    ...
    for (const row of latest) {
        ...
        if (row.status === "closed") {
            closedCount += 1;
            closeDaysSum += row.daysToClose;
        }
        else {
            bucketCounts.set(row.ageBucket, bucketCounts.get(row.ageBucket) + 1);
        }
    }
    return {
        asOf: asOfDate(rows),
        ...
        statusCounts,
        buckets: BUCKET_LABELS.map((label) => ({ label, count: bucketCounts.get(label) })),
        closedCount,
...
function toCleanCsv(rows) {
    const clean = enrich(rows);
    const header = "claim_id,line_of_business,accident_period,report_date,valuation_date,development_month,status,close_date,paid_to_date,case_reserve,incurred,earned_premium,is_latest,age_days,age_bucket,days_to_close";

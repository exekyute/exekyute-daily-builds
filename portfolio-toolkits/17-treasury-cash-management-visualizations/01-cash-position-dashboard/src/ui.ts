...
let currentPositions: AccountPosition[] = [];
 ...
 * colour.
 */
function drawChart(positions: AccountPosition[]): void {
...
function drawTable(positions: AccountPosition[]): void {
  const tbody = el<HTMLTableSectionElement>("positionBody");
  tbody.innerHTML = "";
...
function drawSummary(summary: PositionSummary): void {
...
function init(): void {
  ...
  el<HTMLButtonElement>("exportBtn").addEventListener("click", exportBalances);
}

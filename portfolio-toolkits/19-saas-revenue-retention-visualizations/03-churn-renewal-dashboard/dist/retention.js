...
function parseMoneyToCents(raw, rowNumber, field, allowZero) {
    if (!/^\d+(\.\d{1,2})?$/.test(raw)) {
 ...
 */
function parseMovementCsv(text) {
    const lines = text
        .split(/\r?\n/)
        .map((line) => line.trim())
 ...
 */
function computeRetention(movement) {
 ...
 */
function parseRenewalsCsv(text) {
    const lines = text
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line.length > 0);
 ...
 */
function upcomingRenewals(renewals, asOfMonth, horizonMonths) {

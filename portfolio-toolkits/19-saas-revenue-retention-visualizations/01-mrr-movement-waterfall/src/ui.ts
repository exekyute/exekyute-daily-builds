...
let currentMovement: MovementRow[] = [];
let annualize = false;
...
interface Step {
  label: string;
  // For totals (opening, closing) the bar runs from zero. For deltas it floats
  // from the running base by the signed amount.
  isTotal: boolean;
  amountCents: number; // signed: positive grows the book, negative shrinks it
}

/** The six waterfall steps for one month, opening through closing. */
function stepsFor(row: MovementRow): Step[] {
  return [
 ...
 */
function drawWaterfall(row: MovementRow): void {
...
function drawTable(rows: MovementRow[]): void {
  const tbody = el<HTMLTableSectionElement>("movementBody");
  tbody.innerHTML = "";
  for (const r of rows) {
...
function drawSummary(rows: MovementRow[]): void {
...
function fillMonthPicker(rows: MovementRow[]): void {
  const select = el<HTMLSelectElement>("monthPick");
  select.innerHTML = "";
  rows.forEach((r) => {
...
function redraw(): void {
  ...
  const month = el<HTMLSelectElement>("monthPick").value;
...
function init(): void {
  ...
  el<HTMLSelectElement>("monthPick").addEventListener("change", redraw);
  el<HTMLInputElement>("annualize").addEventListener("change", (event) => {
    annualize = (event.target as HTMLInputElement).checked;
    redraw();
  });
  el<HTMLButtonElement>("exportBtn").addEventListener("click", exportMovement);
}

...
const state: DashboardState = { period: null, aging: null, category: null };
...
function bar(widthPct: number, className: string): string {
...
function renderMetrics(): void {
  ...
  if (state.period) {
    ...
    cards.push(metricCard(
      "Flow identity",
      failures === 0 ? "Balanced" : failures + " off",
      failures === 0 ? "opening + new - closed = closing" : "rows do not balance",
      failures === 0 ? "ok" : "alert",
    ));
  }
  if (state.aging) {
...
function renderTimeToClose(): void {
  if (!state.category) {
...
function ingest(text: string, fileName: string): void {

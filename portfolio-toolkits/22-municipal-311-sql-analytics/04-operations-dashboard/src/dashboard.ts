...
interface AgingRow {
  bucket: string; // "0-7", "8-14", "15-30", "31+"
  openCount: number;
  overdue: number;
}
...
type CsvKind = "period" | "aging" | "category" | "unknown";
...
function parsePeriodRows(table: string[][]): PeriodRow[] {
...
function parseAgingRows(table: string[][]): AgingRow[] {
  const header = table[0];
...
function parseCategoryRows(table: string[][]): CategoryRow[] {
  const header = table[0];
...
function identityHolds(row: PeriodRow): boolean {
  return row.opening + row.newRequests - row.closed === row.closing;
}

function identityFailures(rows: PeriodRow[]): PeriodRow[] {
  return rows.filter((row) => !identityHolds(row));
}

function totalCostCents(rows: PeriodRow[]): number {
  return rows.reduce((sum, row) => sum + row.costToServeCents, 0);
}

function totalOpen(rows: AgingRow[]): number {
  return rows.reduce((sum, row) => sum + row.openCount, 0);
}

function totalOverdue(rows: AgingRow[]): number {
  return rows.reduce((sum, row) => sum + row.overdue, 0);
}

function totalClosed(rows: CategoryRow[]): number {
  return rows.reduce((sum, row) => sum + row.closedCount, 0);
}

function totalBreaches(rows: CategoryRow[]): number {
...
function avgDaysHundredths(totalDaysValue: number, count: number): number {
  if (count === 0) {
    return 0;
  }
...
function overallAvgDaysHundredths(rows: CategoryRow[]): number {
